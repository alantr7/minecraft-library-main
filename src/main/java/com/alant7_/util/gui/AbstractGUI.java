...
public abstract class AbstractGUI {
    ...
    public AbstractGUI(BukkitPlugin plugin, Player player) {
        this(plugin, player, true);
    }

    public AbstractGUI(BukkitPlugin plugin, Player player, boolean init) {
        this.plugin = plugin;
        this.player = player;
        if (init) {
            init();
    ...
    private void _interact(int slot, ClickType type, InventoryClickEvent event) {
        ...
        if (type.isLeftClick()) {
            ...
            this.click(slot, com.alant7_.util.gui.ClickType.MIDDLE);
        } else if (event != null) {
            event.setCancelled(true);
        }
    }
