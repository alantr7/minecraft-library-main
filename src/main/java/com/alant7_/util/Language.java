...
public class Language extends Configuration {
    ...
    private static HashMap<BukkitPlugin, Language> instances = new HashMap<>();

    private Language(BukkitPlugin plugin, String path) {
        super(plugin, path);
    }

    public static void init(BukkitPlugin plugin, String path) {
        instances.put(plugin, new Language(plugin, path));
        instances.get(plugin).load();
    }

    public static String get(BukkitPlugin plugin, String message) {
        String msg = instances.get(plugin).getConfig().getString(message);
        return msg != null ? Formatter.formatColors(msg) : "";
    }

    public static List<String> getList(BukkitPlugin plugin, String message, String[] replace, Object[] replaceWith) {
    ...
    @SafeVarargs
    public static List<String> getList(BukkitPlugin plugin, String message, Pair<String, Object>... pairs) {
        List<String> list = instances.get(plugin).getConfig().getStringList(message);
        List<String> r = new ArrayList<>();
    ...
    public static List<String> getList(BukkitPlugin plugin, String message) {
